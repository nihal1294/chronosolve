...
def validate_problem(problem: TimetableProblem) -> list[ValidationIssue]:
    ...
    """
    # Imported lazily: both check modules import Severity/ValidationIssue from
    # this module, so top-level imports here would be circular.
    from timetable_solver.validation.advanced_checks import check_advanced_rules
    from timetable_solver.validation.pre_assignment_checks import check_pre_assignments

    issues: list[ValidationIssue] = []
    _check_group_hours_feasibility(problem, issues)
    _check_teacher_availability_feasibility(problem, issues)
    check_pre_assignments(problem, issues)
