...
Run the solver and watch it converge live; cancel anytime. An optional **Polish result** toggle adds a refinement pass that keeps improving the timetable after the main solve, with its own live progress readout. When it finishes you get a quality score, the solve status and time, room utilization, and a soft constraint penalty breakdown covering every preference - advanced rules and softened ones included - plus one-click exports: CSV rows, a printable master PDF report, and calendar (ICS) files with weekly events for any teacher or class. If no valid timetable exists, the Scheduler names the exact rules that clash and offers a one-click **Soften to preference**: the rule becomes a weighted preference the next run honors as far as possible instead of a hard barrier, and the quality score shows exactly how well it was kept.
