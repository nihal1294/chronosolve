...
Run the solver and watch it converge live; cancel anytime. When it finishes you get a quality score, the solve status and time, room utilization, and a soft constraint penalty breakdown covering every preference - advanced rules and softened ones included - plus one-click CSV export (PDF and calendar/ICS exports are on the roadmap). If no valid timetable exists, the Scheduler names the exact rules that clash and offers a one-click **Soften to preference**: the rule becomes a weighted preference the next run honors as far as possible instead of a hard barrier, and the quality score shows exactly how well it was kept.
